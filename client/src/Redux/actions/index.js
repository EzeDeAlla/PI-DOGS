...
const getAllDogs = () => {
    return async function (dispatch) {
        try{
            var json = await axios.get('https://dogsbackend-r86h.onrender.com/dogs');
            return dispatch({
                type: GET_ALL_DOGS,
                payload: json.data
            })
...
const getTemperaments = () => {
    return async function (dispatch) {
        try {
            var json = await axios.get('https://dogsbackend-r86h.onrender.com/temperaments')
            return dispatch({
...
const getDog = (id) => {
    return async (dispatch) => {
        let json = await axios.get(`https://dogsbackend-r86h.onrender.com/dogs/${id}`)
        return dispatch({
            type: GET_DOG,
            payload: json.data
        });
...
const getDogByName = (name) => {
    return async (dispatch) => {
            let json = await axios.get(`https://dogsbackend-r86h.onrender.com/dogs?name=${name}`)
            return dispatch({
...
const postDogs =(payload)=>{
    return async function(){
        const create= await axios.post('https://dogsbackend-r86h.onrender.com/dogs',payload);
        return create;
    }
}
